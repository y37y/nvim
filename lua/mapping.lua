...
function M.core_mappings(mappings)
  ...
  if maps then
    ...
    -- Make regular y copy to system clipboard in both modes
    ...
    -- Select from true beginning to true end (g_ goes to last non-blank character)
    maps.n["<leader>a"] = { 
    function()
        vim.cmd('normal! gg0')
        vim.cmd('normal! vG$')
    ...
    -- vertical diff with clipboard
    maps.n["<leader>dv"] = { 
    function()
      -- Create new vertical split
      vim.cmd('vnew')
      -- Paste clipboard content into the new buffer
      vim.cmd('normal! "+P')
      -- Set both buffers to diff mode
